...
4. 完了後、ワークフローの成果物 (Artifacts) から以下をダウンロード:
   ...
   - `utawave-unsigned`（署名対象の `Utawave.exe`）/ `Utawave-<version>-Windows-x64-pdb`（`Utawave.pdb`・手元保管）
...
- **インストーラ配布 (主役)**: `utawave-unsigned` の `Utawave.exe` を署名 → `installer\Utawave.iss`
