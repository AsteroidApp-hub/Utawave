    ...
    : juce::AudioProcessorEditor(e), eq(e)
{
    ...
    binDb.assign((size_t) (BuiltInEQ::kAnalyzerSize / 2 + 1), kAnMinDb);
...
void BuiltInEQEditor::updateAnalyzer()
{
    ...
    const float scale = 4.0f / (float) N;   // 0 dBFS 正弦 ≈ 0 dB になる正規化
    for (int k = 0; k <= N / 2; ++k)
    {
        ...
        float& s = binDb[(size_t) k];
        s = db > s ? db : s * 0.82f + db * 0.18f;   // 立ち上がり即・立ち下がりゆっくり
    }
}
...
void BuiltInEQEditor::paint(juce::Graphics& g)
{
    ...
    {
        juce::Path an;
        an.startNewSubPath(gf.getX(), gf.getBottom());
        const float span = kAnMaxDb - kAnMinDb;
        for (int px = 0; px <= graph.getWidth(); ++px)
        {
            const float x = gf.getX() + (float) px;
            const double f = xToFreq(x);
            int bin = (int) std::lround(f * N / juce::jmax(1.0, sr));
            bin = juce::jlimit(0, N / 2, bin);
            const float db = juce::jlimit(kAnMinDb, kAnMaxDb, binDb[(size_t) bin]);
            const float t = (db - kAnMinDb) / span;
            an.lineTo(x, gf.getBottom() - t * gf.getHeight());
        }
        an.lineTo(gf.getRight(), gf.getBottom());
        an.closeSubPath();
        g.setColour(AppColours::accent.withAlpha(0.18f));
        g.fillPath(an);
    }
