...
public:
    ...
    // 停止時は新録音を Lane 0 に置き、新録音自体だけを Take レーンへバックアップする。
    // 上書きされた Lane 0 の既存クリップは退避しない (上塗り。各録音は録音時に自身の
    // バックアップを残しているため、再退避するとトリム断片がテイクに混入する)
    // fileOffset: 録音ファイル先頭の読み飛ばし秒 (録音レイテンシ補正でタイムライン 0 に
    // クランプされた分。lane0 クリップとテイクバックアップの両方に同値が入る)
