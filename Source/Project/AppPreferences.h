...
public:
    ...
    // フォルダトラックに Pan / Rev を表示するか (既定: 表示)。
    // 「フォルダは M/S/Vol だけのシンプルなまとめ役でいい」ユーザー向けに隠せる。
    // 表示のみの設定で、非表示でも既存の Pan/Rev の値・効果はそのまま生きる。
    // INS スロットは他トラックとレイアウトを揃えるためフォルダでも常に表示 (この設定の対象外)。
    bool showFolderPanRev { true };
    ...
    // ── 配信ミラー出力 (ハードウェア依存のためアプリ全体設定) ──
    // 最終ミックス (再生 + INS を通ったモニタ返し) をメインとは別の出力デバイスへ同時に
    // 流すか (既定: OFF)。主目的は配信: メイン出力が ASIO 等で配信ソフトから見えない環境でも、
    // ミラー側が WASAPI 共有 (Mac は CoreAudio) のセッションを作るので配信ソフトの
    // アプリ音声キャプチャで拾える (ループバック機能のないインターフェース向け)。
    bool streamMirrorEnabled { false };
    // ミラーの出力先デバイス名 (空 = 未選択 = ミラーは開始しない)。
    // 「空 = OS 既定の出力デバイス」の旧仕様は廃止 (2026-07): 既定 = メイン出力と同じ I/O の
    // 環境が大半で、黙って同じデバイスへミラーすると二重聞こえ事故になるため。
    juce::String streamMirrorDevice;

    // ── アプリケーショントラック (アプリ音声取り込み・歌枠向け・Windows のみ実装) ──
    // 「+ トラック追加」メニューに「アプリケーショントラックを追加」を出すか (既定: OFF)。
    // アプリケーショントラック = 指定アプリ (ブラウザ / カラオケアプリ等) の音をトラックとして
    // ライブで混ぜる (Vol/Pan/Mute/Solo が効く・録音/書き出しには乗らない)。OFF でも既存の
    // アプリケーショントラックはそのまま動く (追加導線が消えるだけ = enableFolderTracks と同じ)。
    bool enableAppCaptureTracks { false };
