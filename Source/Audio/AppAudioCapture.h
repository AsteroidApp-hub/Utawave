...
public:
    ...
    // ── リリースゲート (公開タイミングの制御) ──
    // 2026-07-19 公開 (2000DL 突破の勢いに乗せるためユーザー判断で前倒し)。false にすると
    // 環境設定のトグルと「+ トラック追加」メニューが隠れる (エンジン/トラック/保存復元は
    // 生きたままなので、既存プロジェクトのアプリケーショントラックは動き続ける)。
    static constexpr bool releaseUnlocked = true;
