...
public:
    ...
    // ── リリースゲート (公開タイミングの制御・ユーザー決定 2026-07-18) ──
    // アプリケーショントラックは「次の次のリリース」で公開する (直近のリリースは MIDI 強化が
    // 主対象のため、機能は実装済みのまま導線を隠しておく)。**公開時はこれを true にするだけ**:
    // 環境設定のトグルと「+ トラック追加」メニューが現れる。false の間もエンジン/トラック/
    // 保存復元は生きているので、テストビルドで作ったプロジェクトはそのまま開けて鳴る
    // (追加導線が無いだけ)。公開時は help.html 5 言語への使い方追記とリリースノートも忘れない
    // こと (Docs/internal/PLAN_アプリ音声取り込み.md の公開チェックリスト参照)。
    static constexpr bool releaseUnlocked = false;
